# app.py — Kits Inventory (full CRUD, Excel, Handover & KIT FORM)
from flask import (
...
# ───────────────────────────────────────────────────────────── Config
DB_PATH = r"f:/Sai_Python/test_pd/kits_inventory.db"
...
def update_from_dict(kit, data: dict):
...
def export_excel():
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        df.to_excel(w, index=False, sheet_name="Kits Inventory")
    buf.seek(0)
...
# ───────────────────────────────────────────────────────────── Front-end
@app.route("/")
def index():  return render_template("kits_inventory.html")
